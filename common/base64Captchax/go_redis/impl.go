package go_redis

import (
	"context"
	"github.com/SpectatorNan/goutils/common/base64Captchax"
	"github.com/mojocn/base64Captcha"
	"github.com/redis/go-redis/v9"
	"time"
)

type redisStore struct {
	redis      *redis.Client
	expiration time.Duration
	prefix     string
}

func NewRedisStore(redis *redis.Client, expiration int, prefix string) base64Captcha.Store {
	s := new(redisStore)
	s.redis = redis
	s.expiration = time.Second * time.Duration(expiration)
	s.prefix = prefix
	return s
}

func (s *redisStore) SetCaptcha(prefix string, id string, value string) error {
	cacheKey := base64Captchax.GenerateKey(prefix, id)
	return s.Set(cacheKey, value)
}
func (s *redisStore) GetCaptcha(prefix string, id string, clear bool) string {
	cacheKey := base64Captchax.GenerateKey(prefix, id)
	return s.Get(cacheKey, clear)
}
func (s *redisStore) VerifyCaptcha(prefix string, id, answer string, clear bool) bool {
	cacheKey := base64Captchax.GenerateKey(prefix, id)
	return s.Verify(cacheKey, answer, clear)
}

func (s *redisStore) Set(id string, value string) error {
	return s.redis.SetEx(context.Background(), id, value, s.expiration).Err()
}
func (s *redisStore) Verify(id, answer string, clear bool) bool {
	v := s.Get(id, clear)
	return v == answer
}
func (s *redisStore) Get(id string, clear bool) (value string) {
	value, err := s.redis.Get(context.Background(), id).Result()
	if err != nil {
		return
	}
	if clear {
		_, _ = s.redis.Del(context.Background(), id).Result()
	}
	return
}
