...
import (
	"github.com/SpectatorNan/goutils/common/imagex/vipsx/export"
...
func webpEncoder(img *vips.ImageRef, quality int) ([]byte, error) {
	...
	if quality >= 100 {
		...
		//	StripMetadata: true,
		//})
		buf, err = export.ExportWebpWithQuality(img, true, 0, true, false, 0)
	} else {
		...
		for i := 0; i < 7; i++ {
			...
			buf, err = export.ExportWebpWithQuality(img, true, quality, false, false, i)
			if err == nil {
				break
			}
		}
...
func jxlEncoder(img *vips.ImageRef, quality int) ([]byte, error) {
	...
	if quality >= 100 {
		...
		//	Distance: 1.0,
		//})
		buf, err = export.ExportJxlWithParams(img, 0, true, 4, 1.0, 1)
	} else {
		//buf, _, err = img.ExportJxl(&vips.JxlExportParams{
		//	Effort:   1,
		//	Tier:     4,
		...
		//	Distance: 1.0,
		//})
		buf, err = export.ExportJxlWithParams(img, quality, false, 4, 1.0, 1)
	}
...
func avifEncoder(img *vips.ImageRef, quality int, stripeMeta bool) ([]byte, error) {
	...
	if quality >= 100 {
		...
		//})
		buf, err = export.ExportAvifWithParams(img, stripeMeta, 0, 0, 0, true)
	} else {
		//buf, _, err = img.ExportAvif(&vips.AvifExportParams{
		//	Quality:       quality,
		//	Lossless:      false,
		//	StripMetadata: stripeMeta,
		//})
		buf, err = export.ExportAvifWithParams(img, stripeMeta, quality, 0, 0, false)
	}
